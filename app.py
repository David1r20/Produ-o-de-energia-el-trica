...
def main():
    ...
    try:
        ...
        # Avaliar os modelos
        resultados = avaliar_modelos(X_train, X_test, y_train, y_test)

        st.subheader("Avaliação dos Modelos Ajustados")
        st.write(f"**Modelo Lasso Regression (Melhor Alpha: {resultados['lasso']['alpha']}):**")
        st.write(f"   Mean Squared Error (MSE): {resultados['lasso']['mse']:.2f}")
        st.write(f"   R-squared (R2): {resultados['lasso']['r2']:.2f}")

        st.write(f"**Modelo Ridge Regression (Melhor Alpha: {resultados['ridge']['alpha']}):**")
        st.write(f"   Mean Squared Error (MSE): {resultados['ridge']['mse']:.2f}")
        st.write(f"   R-squared (R2): {resultados['ridge']['r2']:.2f}")
        ...
        predicted_energy_output_lasso = resultados['lasso']['model'].predict(input_data_scaled)[0]
        predicted_energy_output_ridge = resultados['ridge']['model'].predict(input_data_scaled)[0]
